...
- [x] **M12 — Linux manual verification checklist:** deb install+launch, AppImage launch, folder picker, MP3+MP4 lipsync, avi auto-transcode, cover fallback chain, phone remote over LAN (after `ufw allow`), loudness normalization, F11 fullscreen, pause/quit overlay, codec-missing hint on minimal-GStreamer VM.
  *Verify:* checklist green on an Ubuntu/Debian VM; Fedora best-effort only.
  - **Confirmed working on real Ubuntu 24.04 VM (2026-07-15):** deb install+launch, folder picker, refresh library, cover fallback chain (`#COVER`→fuzzy-extension→`#BACKGROUND` tiers all verified with real/edited corpus files), phone remote over LAN (once the VM's network adapter was correctly bridged — see gotcha below), loudness normalization, F11 fullscreen, pause/Esc quit overlay (minor known caveat: brief audio-buffer drain after pause, see Known-open), MP3+MP4 lipsync (after the local-HTTP-server fix below), video playback (after the same fix). AppImage launch and avi auto-transcode specifically not yet exercised on this VM.
  - **Bug found and fixed:** the `.deb` package failed to coexist with the system `ffmpeg` package — Tauri's deb bundler installs `externalBin` sidecars into `/usr/bin` by their base name, so the `ffmpeg` sidecar collided with `/usr/bin/ffmpeg` from Ubuntu's real `ffmpeg` package (`dpkg` refused install/upgrade with "trying to overwrite ... which is also in package karaoke"). Fixed by renaming the sidecar identifier from `ffmpeg` to `karaoke-ffmpeg` everywhere: `tauri.conf.json` externalBin, `capabilities/default.json` shell allow-execute, `transcode.ts`'s `Command.sidecar()` call, both release scripts, and README.
  - **Major bug found and fixed: audio/video playback desync**, see the Progress log entry below for the full root-cause chain (`GST_DEBUG` investigation → local-HTTP-server fix). User-confirmed resolved on two previously-broken songs (one video, one audio-only).
  - **Gotcha (not an app bug): VM networking for the phone remote.** With multiple virtual adapters on the host (e.g. Hyper-V's default switch alongside a real NIC), VMware's "Bridged (Automatic)" can silently bridge to the wrong one, giving the VM an IP on an unreachable virtual subnet instead of the real LAN — phone remote then can't be reached from a phone even though the app is working correctly. Fix is on the host: VMware's Virtual Network Editor → VMnet0 → "Bridged to:" → explicitly pick the real physical adapter, not "Automatic" or an obviously-virtual one; then renew the VM's DHCP lease (`sudo nmcli connection down ens33 && sudo nmcli connection up ens33`, interface name may vary) or reboot the VM.
...
**Version 0.8.0 shipped (Windows) — Linux support M8–M12 all done and real-VM-verified as of 2026-07-15.** `dist\Karaoke_0.8.0_x64-setup.exe` (28.8 MB) + `dist\Karaoke_0.8.0_portable.zip` (40.8 MB) on Windows; a real `.deb` was built, installed, and manually verified on an Ubuntu 24.04 VMware VM (not yet published as a versioned release artifact — see Known-open). All milestones M0–M12 done, plus all 12 items from the 2026-07-06 feedback backlog. 141 tests green (`npm test`). Release via `.\scripts\release.ps1` (auto-updates the ffmpeg sidecar from gyan.dev each run, progress-bar-suppressed for speed; works in PowerShell 5.1 and 7) and `scripts\release.sh` (Linux, now verified working — fetches the BtbN ffmpeg sidecar and produces working `.deb`+`.AppImage`). ffmpeg sidecar renamed `ffmpeg` → `karaoke-ffmpeg` (avoids a `.deb`/system-package name collision on Linux) — affects both platforms' binary filenames. Publisher: "Weebs Software Inc.". App icon from `assets\icon-source.jpg` (rounded corners; regenerate: `npm run tauri icon assets/icon-source.jpg`). Rust edition 2024. GPLv3 licensed; personal identifier removed from `tauri.conf.json`; README expanded with full feature list + Claude Code dev note + Linux sections.
...
- Linux: songs with a large `#VIDEOGAP` (e.g. Matsumoto Bon Bon, 23.5s) show a few seconds of black-frame flashing while the video seeks to line up with audio at song start, before settling. Real video file plays cleanly in VLC — this is specific to the initial big seek needed to match `#VIDEOGAP`, on this GStreamer/WebKitGTK setup. Tried and reverted: rate-limiting/gating the corrective seek, a currentTime-based "looks stable" heuristic, a fixed real-time hold behind a cover-image overlay gated by gap size — none fully fixed it, and the overlay approach had side effects (10s hold, background not showing on rewind). Accepted as a known cosmetic issue for now; revisit later. The one part of this investigation that's kept: the video drift-correction's rate-limiter now uses wall-clock time instead of the media clock, which also fixed a real, separate bug (video didn't follow the audio backward on a rewind — the media clock moving backward was blocking further corrections).
...
- Clean-VM installer test before wide sharing (Windows); Linux `.deb`/`.AppImage` not yet published as a versioned release artifact — only manually built/tested during M12
- Linux: AppImage launch and avi auto-transcode specifically not yet exercised (deb covered the rest of the M12 checklist); Fedora/RPM deferred — patent-codec licensing (RPM Fusion) adds friction beyond this pass's scope
...
- 2026-07-15 — M12 real Linux verification underway (Ubuntu 24.04 on VMware, not Hyper-V as originally planned). `.deb`/`.AppImage` built and installed successfully on first try. Found and fixed real bugs surfaced only by actual Linux hardware/webview testing:
  - **`.deb` install collision:** Tauri's deb bundler installs `externalBin` sidecars into `/usr/bin` by their base name, so the `ffmpeg` sidecar collided with Ubuntu's real `ffmpeg` package (`dpkg` refused install: "trying to overwrite ... which is also in package karaoke"). Renamed the sidecar to `karaoke-ffmpeg` everywhere (`tauri.conf.json`, capabilities, `transcode.ts`, both release scripts, README).
  - **mp3 transcode fallback used VBR** (`-q:a 2`) — switched to CBR (`-b:a 192k`); VBR mp3s lack a reliable byte-offset-to-time mapping, which GStreamer's decoder handles worse than Chromium.
  - **mp4 transcode fallback missing `-movflags +faststart`** — a `.karaoke.mp4` played fine in VLC (reads the whole file) but failed in the app's range-streamed `<video>`; the moov atom needs to be at the front for range-based readers.
  - **Major bug: audio/video playback desync.** Multiple unrelated songs (both untouched originals and freshly-transcoded files) played audio that jumped forward mid-song a few seconds in, while on-screen lyrics/timer stayed correctly aligned with `currentTime` — proving the decoded audio samples themselves were out of position, not our timing math. Ruled out via testing: file corruption (gst-play-1.0 and VLC play the same file cleanly), VBR encoding, missing mp4 faststart index, CPU/RAM starvation (VM has 4-core/8-thread + 8GB), audio-driver errors (`journalctl` showed nothing), and blob-URL loading (tried as a fix, made it worse — Tauri's blob URLs are proxied as `blob:tauri://localhost/...`, not true in-memory Blobs, so they hit the same code path). `GST_DEBUG=3` logs during actual playback found the real cause: GStreamer's `queue2` buffering element logging "custom-downstream-sticky event can't be added to temp file" while fielding a burst of out-of-order HTTP Range requests against Tauri's custom protocol scheme, followed by `mpegaudioparse` repeatedly losing frame sync ("invalid bitrate"/"invalid samplerate") throughout playback. Fixed by routing audio/video through the app's own local axum HTTP server instead (new `/media/audio`, `/media/video` routes using `tower_http::services::ServeFile` for proper Range support, backed by a new `set_now_playing_media` Tauri command; gated to Linux only via `isLinux()`, Windows keeps the asset protocol unchanged). **User-confirmed fixed** on the test VM: both a previously-broken video song and a previously-broken audio-only song now play correctly, video included.
  - Also fixed unrelated to Linux: `tests/corpus.test.ts` now skips gracefully instead of failing when `Research/songs` isn't present (untracked, large media files — was failing on the fresh VM clone).
  - Verified on Windows throughout: 141 tests green, `svelte-check` clean, `cargo check`/`cargo clippy` clean, `npm run tauri dev` launches and runs with no regression after every change.
  - Still open: rest of the M12 manual checklist (folder picker, cover fallback chain, phone remote over LAN, loudness normalization, F11 fullscreen, pause/quit overlay, codec-missing hint on a minimal-GStreamer VM) not yet run through end-to-end on this VM.
- 2026-07-15 — Rest of the M12 checklist run to completion on the same Ubuntu 24.04 VM: folder picker, cover fallback chain (verified all three tiers using a mix of real corpus quirks and a deliberately-edited chart — fuzzy-extension match, `#BACKGROUND` fallback), phone remote over LAN, loudness normalization, F11 fullscreen, and pause/Esc quit overlay all confirmed working. Also added a **Refresh library** button (`SongList.svelte`/`+page.svelte`) after noticing there was no way to pick up added/removed songs post-startup without reopening the folder-picker dialog — user tested it live by moving songs out of the real corpus folder and back. `release.ps1` also got a fix: `$ProgressPreference = "SilentlyContinue"` before its `Invoke-WebRequest` calls, since PowerShell's default progress-bar rendering was making the ffmpeg download dramatically slower than the actual network speed. Two minor non-blocking findings: (1) pausing on Linux leaves audio playing for a brief moment before going silent — confirmed absent on Windows with identical code, points to GStreamer/PulseAudio sink buffer drain latency rather than an app bug; (2) VMware's "Bridged (Automatic)" networking silently bridged to a Hyper-V virtual adapter instead of the real NIC, blocking the phone remote until manually corrected in VMware's Virtual Network Editor — a VM-config gotcha, not an app bug. **M8–M12 all done; Linux support is real and verified**, pending only a proper versioned release publish.
