...
- **Linux audio/video playback desync (2026-07-15, unresolved, blocks M12):** on the real Ubuntu 24.04 VMware VM used for M12 testing, audio playback jumps forward mid-song a few seconds after starting (e.g. plays 2-3s in, then audibly skips to ~88s in) on multiple, unrelated songs — reproduces on both an untouched original mp3 (with unusual embedded metadata) and a freshly ffmpeg-CBR-transcoded clean mp3, ruling out file-specific corruption. Video (mp4, `+faststart` already applied) also fails to display in some songs. Root-caused as far as remote debugging allowed:
  - The song's own file is fine: `gst-play-1.0` and VLC both play it start-to-finish with no skip.
  - On-screen lyrics/HUD timer stay perfectly aligned with `<audio>`'s `currentTime` throughout — ruling out a timing-math bug in `clock.ts`/`Sing.svelte`'s render loop.
  - The *decoded audio samples* audibly jump ahead of what `currentTime` reports — the WebKitGTK/GStreamer media pipeline is playing back the wrong position while its own position-reporting stays smooth. This points to a WebKitGTK/GStreamer decode-position bug in this environment, not app code.
  - Ruled out: VBR encoding (switched to CBR, no change), missing MP4 faststart index (added, video still fails), Tauri asset-protocol HTTP-range streaming (tried routing audio through blob URLs instead — made it *worse*, immediate jump instead of after 2-3s, then reverted), CPU/RAM starvation (VM has 4-core/8-thread + 8GB, generous), audio-driver errors (`journalctl -f` during playback showed no xrun/ALSA/PulseAudio errors).
  - Not yet tried: checking installed `webkit2gtk`/GStreamer package versions for known bugs, WebKitGTK env var workarounds (e.g. disabling compositing/hardware accel), or testing the same `.deb` on a different Linux install/hypervisor (bare metal, VirtualBox) to see if it's VMware-specific.
  - Fixes that *did* land and are believed correct regardless: `karaoke-ffmpeg` sidecar rename (fixed a real `.deb`/system-ffmpeg package collision), CBR mp3 transcode (still a correctness improvement even though it didn't fix this bug), `+faststart` mp4 transcode flag (still correct for range-streamed playback in general).
